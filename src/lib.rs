...
use crate::requests::{QueuedRequestBuilder, RequestDelay, RequestQueue};
/// Reexport all the scraper types
pub use scraper;
//
// /// Collector controls the `Crawler` and forwards the successful requests to
// the /// `Scraper`.
...
//     /// Create a new `Collector` that uses the `scraper` for content
// extraction     pub fn new(scraper: T, config: CrawlerConfig) -> Self {
//         Self {
...
// Poll<Option<Self::Item>> {         let pin = self.get_mut();
//
//         loop {
//             match pin.crawler.poll(cx) {
//                 Poll::Ready(Some(result)) => match result {
//                     CrawlResult::Finished(Ok(output)) => return
...
//
//                         let output = pin.scraper.scrape(response, &mut
// pin.crawler);
//
//                         pin.crawler.current_depth = 0;
//
//                         match output {
//                             Ok(Some(output)) => return
// Poll::Ready(Some(Ok(output))),                             Err(err) => return
// Poll::Ready(Some(Err(err))),                             _ => {}
//                         }
//                     }
//                     CrawlResult::Crawled(Err(err)) => return
...
// String>>>>;
//
// /// The crawler that is responsible for driving the requests to completion
// and /// providing the crawl response for the `Scraper`.
...
//     /// Futures that eventually result in `T::Output` and are piped directly
// to     /// caller
...
//     request_queue: VecDeque<QueuedRequestBuilder<T::State>>,
...
//     /// Delay to apply for requests if whitelist is empty
...
//
//         let mut allowed_domains =
// HashMap::with_capacity(config.allowed_domains.len());         let mut
// allowed_domains_keys = Vec::with_capacity(config.allowed_domains.len());
//
...
// config.skip_non_successful_responses,             url_filter:
// config.url_filter,         }
//     }
// }
//
...
//     <T as Scraper>::Output: Unpin,
// {
//     /// Send a crawling request whose html response and context is returned
// to     /// the scraper again
//     pub fn crawl<TCrawlFunction, TCrawlFuture>(&mut self, fun:
// TCrawlFunction)     where
//         TCrawlFunction: FnOnce(Arc<reqwest::Client>) -> TCrawlFuture,
//         TCrawlFuture: Future<Output = Result<(reqwest::Response,
...
//             Ok(Response {
//                 depth,
//                 // Note: There is no way to determine the original url since
// only the response is                 // returned from the future So we set
// the `request_url = response_url`                 request_url: url.clone(),
//                 response_url: url,
...
//     }
//
//     /// Submit a complete crawling job that is driven to completion and
// directly     /// returned once finished.
//     pub fn complete<TCrawlFunction, TCrawlFuture>(&mut self, fun:
// TCrawlFunction)     where
//         TCrawlFunction: FnOnce(Arc<reqwest::Client>) -> TCrawlFuture,
...
//     }
//
//     /// This queues in a GET request for the `url`, without any state
// attached     pub fn visit(&mut self, url: impl IntoUrl) {
...
//         self.request_with_state(self.client.request(reqwest::Method::GET,
// url), state)     }
//
...
//     pub fn request_with_state(&mut self, req: reqwest::RequestBuilder, state:
// T::State) {         self.queue_request(req, Some(state))
//     }
//
//     fn queue_request(&mut self, request: reqwest::RequestBuilder, state:
// Option<T::State>) {         let req = QueuedRequestBuilder {
//             request,
//             state,
...
//     }
//
//     /// The crawler's client is wrapped within an `Arc`, so this can be used
...
//     pub fn remaining_delay(&self, domain: &str, now: Instant) ->
// Option<Duration> {         if let Some(domain_delay) =
// self.allowed_domains.get(domain) {             return
// domain_delay.next_delay(now);         }
//         self.request_delay().map(|d| d.delay())
//     }
//
//     fn get_response(&self, request: BufferedRequest<T::State>) ->
// CrawlRequest<T::State> {         let BufferedRequest {
//             request,
//             state,
//             depth,
//         } = request;
...
//             .send()
//             .map_err(move |_| err)
//             .and_then(|resp| RobotsHandler::from_response(resp).map_err(move
// |_| host));
//
...
//                     next_request = Some((req, self.request_delay().map(|d|
// d.delay())))                 } else {
...
//                 }
//             } else if
// self.in_progress_robots_txt_crawl_hosts.contains_key(&host) {
// self.buffered_until_robots.push_back(req);             } else {
...
// domain.queue_mut().push_back(req);             } else {
//                 // robots txt not requested yet for this
//                 // request's host
//                 self.get_robots_txt(req.request.url().clone(), host,
// key.clone());                 domain.is_waiting_for_robots = true;
//                 domain.queue_mut().push_back(req);
//             }
//         } else {
//             domain.queue_mut().push_back(req);
...
//             }
//
//             // drain submitted futures by removing them one by one and add
// them back if not             // ready
//             for n in (0..self.in_progress_complete_requests.len()).rev() {
//                 let mut request =
// self.in_progress_complete_requests.swap_remove(n);                 if let
// Poll::Ready(resp) = request.poll_unpin(cx) {                     match resp {
//                         Ok(Some(output)) => {
//                             self.queued_results
//
...
//             }
//
//             // drain the crawl futures by removing them one by one and add
// them back if not             // ready
//             for n in (0..self.in_progress_crawl_requests.len()).rev() {
//                 let mut request =
// self.in_progress_crawl_requests.swap_remove(n);                 if let
// Poll::Ready(resp) = request.poll_unpin(cx) {
// self.queued_results.push_back(CrawlResult::Crawled(resp));                 }
// else {                     self.in_progress_crawl_requests.push(request);
//                 }
//             }
//
...
//                 let mut request =
// self.in_progress_robots_txt_crawls.swap_remove(n);                 if let
// Poll::Ready(resp) = request.poll_unpin(cx) {                     match resp {
//                         Ok(robots) => {
...
//                                 .remove(&robots.host)
//                                 .and_then(|domain|
// self.allowed_domains.get_mut(&domain))                             {
...
// self.allowed_domains.remove_entry(&domain_id) {                     if
// !domain.is_waiting_for_robots {                         while
// self.in_progress_crawl_requests.len() < self.max_requests {
// let now = Instant::now();                             if let Some(req) =
// domain.poll(now) {
// self.execute_buffered_request(req, None, cx);
// domain.set_requested_timestamp(now);                             } else {
//                                 break;
//                             }
//                         }
//                     }
...
//                 let now = Instant::now();
//
//                 if let Some(QueuedRequestBuilder {
//                     request,
//                     state,
//                     depth,
...
//
// self.queued_results.push_back(CrawlResult::Crawled(Err(
// CrawlError::ReachedMaxDepth {                                 request,
//                                 state,
//                                 depth,
//                             }
//                             .into(),
...
// req.request.url().domain().and_then(|d| {
// req.request                                     .url()
//                                     .host_str()
//                                     .map(|h| (d.to_lowercase(),
// h.to_lowercase()))                             }) {
...
//                                     // no domains whitelisted, all domains
// that are not blacklisted                                     // are allowed
//                                     if
// self.disallowed_domains.contains(&domain) {
// // domain is blacklisted
// self.queued_results.push_back(CrawlResult::Crawled(Err(
// CrawlError::DisallowedRequest {
// request: req.request,                                                 state:
// req.state,                                                 reason:
// DisallowReason::User,                                             }
//                                             .into(),
//                                         )));
...
//                                     }
//                                 } else {
//                                     // the domain is not whitelisted and the
// request therefore                                     // rejected
//
// self.queued_results.push_back(CrawlResult::Crawled(Err(
// CrawlError::DisallowedRequest {
// request: req.request,                                             state:
...
//
// self.queued_results.push_back(CrawlResult::Crawled(Err(
// CrawlError::InvalidRequest {                                         request:
// req.request,                                         state: req.state,
//                                     }
//                                     .into(),
//                                 )));
//                             }
...
//                                 self.execute_buffered_request(req, delay,
// cx);                             }
//                         }
//                         Err(error) => {
...
//                 }
//             }
//             // If no new results have been queued either, signal `NotReady`
...
//     /// A submitted request to produce the `Scraper::Output` type has
// finished     Finished(Result<T::Output>),
//     /// A page was crawled
//     Crawled(Result<Response<T::State>>),
// }
//
// fn response_info(resp: &mut reqwest::Response) -> (StatusCode, Url,
// HeaderMap) {     let mut headers = HeaderMap::new();
...
// crawler /// with additional requests to visit and drive the scraper's model
// to /// completion.
...
//     /// `State`, pass the state of the `response` along with the request to
// the     /// `Crawler`.
//     fn scrape(
//         &mut self,
